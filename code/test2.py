...
# type of output_sensitive and output_permissible
print('')
print(type(output_sensitive))
print(type(output_permissible))
print('')
...
model = tf.keras.Model(inputs=[input_sensitive, input_permissible],
                       outputs=[final_output, output_sensitive, output_permissible])


# Custom loss function
def custom_loss(penalty_weight_norm=1, penalty_weight_cosine=1):
    def loss(y_true, y_pred):
        # Assuming y_pred is structured as: [final_output_pred, output_sensitive,
        # output_permissible]
        final_output_pred = y_pred[0]
        output_sensitive = y_pred[1]
        output_permissible = y_pred[2]

        # Base binary crossentropy loss
        base_loss = tf.keras.losses.binary_crossentropy(y_true, final_output_pred)

        # Ensure output_sensitive and output_permissible have at least 2 dimensions
        if len(output_sensitive.shape) == 1:
            output_sensitive = tf.expand_dims(output_sensitive, -1)
        if len(output_permissible.shape) == 1:
            output_permissible = tf.expand_dims(output_permissible, -1)

        # Compute penalties
        penalty_norm = tf.reduce_mean(tf.norm(output_sensitive, ord=2, axis=1))
        penalty_cosine = -tf.reduce_mean(tf.keras.losses.cosine_similarity(output_sensitive, output_permissible, axis=1))

        # Total loss
        return base_loss + penalty_weight_norm * penalty_norm + penalty_weight_cosine * penalty_cosine
...
model.compile(optimizer='adam',
              loss=custom_loss(penalty_weight_norm=0.1, penalty_weight_cosine=0.1),
              metrics=['accuracy'])
...
# Correctly accessing the predictions if model.predict returns a list
prediction_output = model.predict([X_test_sensitive, X_test_permissible])
y_pred = (prediction_output[0] > 0.5).astype(int)  # Adjust index based on your model's specific output structure
y_pred = np.round(y_pred).flatten()
y_true = y_test.values
sensitive_column = X_test_sensitive[:, 0]

# Your existing code for metrics calculation follows here...
