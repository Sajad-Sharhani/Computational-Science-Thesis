...
for beta in betas:
    ...
    metrics_by_group.append(metric_frame.by_group)
...
# Plot metrics by group for each beta
for beta, metrics in zip(betas, metrics_by_group):
    metrics.plot(kind='bar', figsize=(12, 5), ylim=[0, 1])
    plt.title(f'Metrics by Group for Beta = {beta}')
